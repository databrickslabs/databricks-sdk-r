...
external_locations_create <- function(name, url, credential_name, comment = NULL,
  ...
  .state$api$do("POST", "/api/2.1/unity-catalog/external-locations", body = body)
}
...
external_locations_delete <- function(name, force = NULL) {
  ...
  .state$api$do("DELETE", paste("/api/2.1/unity-catalog/external-locations/", name,
    sep = ""), query = query)
}
...
external_locations_get <- function(name) {
  ...
  .state$api$do("GET", paste("/api/2.1/unity-catalog/external-locations/", name,
    sep = ""))
}
...
external_locations_list <- function() {
  ...
  json <- .state$api$do("GET", "/api/2.1/unity-catalog/external-locations")
...
external_locations_update <- function(name, comment = NULL, credential_name = NULL,
  ...
  .state$api$do("PATCH", paste("/api/2.1/unity-catalog/external-locations/", name,
    sep = ""), body = body)
}
